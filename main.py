    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                headers = {'x-access-token': f'{cptoken}', 'api-version': '50', 'app-version': '1.4.152.1', 'device-type': 'ANDROID', 'device-id': 'dummy-id', 'User-Agent': 'okhttp/4.9.1'}
...
async def handle_appx_login_testing(client, message: Message):
    ...
    try:
        ...
        if len(parts) >= 3:
            ...
            msg = await message.reply_text("🔄 Processing Login...")
            
            login_resp = appx_api.appx_login(base_url, email, password)
            if not login_resp.get("success"):
                await msg.edit_text(f"❌ Login Failed: {login_resp.get('error')}")
                return
                
            data = login_resp.get("data", {})
            user_id = data.get("userid") or data.get("user_id") or data.get("id")
            token = data.get("token") or data.get("jwt") or data.get("auth_token") or "No Token Found"
            org_name = data.get("orgName") or data.get("organizationName") or base_url.split('//')[-1].split('.')[0].replace('api', '').capitalize()
            
            courses_resp = appx_api.appx_get_courses(base_url, token, user_id)
            
            out_str = f"**{org_name} Login Successful ✅**\n\n"
            out_str += f"**ID Password :-** `{email}*{password}`\n\n"
            out_str += f"`{token}`\n\n"
            out_str += f"**BATCH ID ➤ BATCH NAME**\n\n"
            ...
            if courses_resp.get("success"):
                ...
                for c in courses:
                    ...
                    c_name = c.get("title") or c.get("course_name") or "Unknown Batch"
                    c_price = c.get("price") or c.get("amount") or "0"
                    if str(c_id) not in batch_ids:
                        batch_ids.append(str(c_id))
                    out_str += f"`{c_id}` - **{c_name}** ✳️ ₹{c_price}\n\n"
                
                if batch_ids:
                    out_str += f"`{','.join(batch_ids)}`\n"
            else:
                out_str += "No courses found or failed to fetch courses.\n"
...
async def text_handler(bot: Client, m: Message):
    ...
    try:
            ...
            if "acecwply" in url:
                ...
                headers = {'x-access-token': f'{raw_text4.strip()}', 'api-version': '50', 'app-version': '1.4.152.1', 'device-type': 'ANDROID', 'device-id': 'dummy-id', 'User-Agent': 'okhttp/4.9.1'}
                c_org = get_orgcode_from_token(raw_text4.strip())
                if c_org: headers['orgcode'] = c_org
                params = {"url": f"{url}"}
                try:
                    response = requests.get('https://api.classplusapp.com/cams/uploader/video/jw-signed-url', headers=headers, params=params)
